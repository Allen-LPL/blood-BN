...
@Data
public class BloodCollectionFactAggReqVO {

    @Schema(description = "过滤条件")
    @Valid
    private BloodCollectionFactQueryReqVO filter;

    @Schema(description = "分组维度列表（支持编码、枚举名或别名）")
    @Size(max = 2, message = "groupBy 最多支持 2 个")
    @InEnum(value = GroupByFieldEnum.class, message = "groupBy 必须是 {value}")
    @JsonDeserialize(contentUsing = GroupByFieldCodeDeserializer.class)
    ...
    public static class MetricSpec {
        ...
        @Schema(description = "聚合字段编码：1-STAR,2-donation_code,3-archive_id,4-base_unit_value,5-age", requiredMode = Schema.RequiredMode.REQUIRED)
        @InEnum(value = MetricFieldEnum.class, message = "field 必须是 {value}")
